...
@Mojo(name = "generate-binary-ebuild")
public class GenerateBinaryEbuildMojo extends AbstractMojo {
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		try (Writer out = new FileWriter(metadataFile.toFile())) {
			...
			throw new MojoFailureException("Could not render the metadata file for parent project " + project.toString(), e);
		}
	}
	...
	private Map<String, Object> createEbuildModel(MavenProject project) throws LicenseNotFoundException, MojoFailureException {
		...
		model.put("src_uri", mavenRepoBase + getJarPath(project));
		model.put("jarFileName", project.getArtifactId() + "-" + project.getVersion() + ".jar");
